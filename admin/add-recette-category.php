...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($nom_categorie) && !empty($sous_categorie)) {
        // Nettoyage des valeurs pour éviter les injections
        ...
        // Requête d'insertion dans la table `categorie_epice`
        $sql = "INSERT INTO categorie_epice (nom_categorie, sous_categorie) VALUES ('$nom_categorie', '$sous_categorie')";
        if (mysqli_query($conn, $sql)) {
            $message = "<p class='text-green-500 text-center'>✅ Catégorie ajoutée avec succès !</p>";
          // Attendre quelques secondes (par exemple, 3 secondes) avant de recharger la page
          echo "<script>
              setTimeout(() => {
