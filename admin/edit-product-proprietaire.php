...
// Vérifier que l'épice appartient bien au propriétaire connecté
$sql = "SELECT * FROM epicerie WHERE id = ? AND proprietaire_id = ?";
...
if (mysqli_num_rows($result) === 0) {
    die("Accès refusé. Vous ne pouvez modifier que vos propres épices.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_sql = "UPDATE epicerie SET nom_epice = ?, poids_net = ?, prix = ?, categorie_id = ?, epicerie_nom = ?, horaires = ?, adresse = ?, contact_epicerie = ?, disponibilite = ?, mots_cles = ?, details = ?, image_epice = ? WHERE id = ? AND proprietaire_id = ?";
    $stmt = mysqli_prepare($conn, $update_sql);
    mysqli_stmt_bind_param($stmt, 'sdsissssssssii', $nom_epice, $poids_net, $prix, $categorie_id, $epicerie_nom, $horaires, $adresse, $contact_epicerie, $disponibilite, $mots_cles, $details, $image_epice, $id, $user_id);

    if (mysqli_stmt_execute($stmt)) {
        $message = '<p class="text-green-500" id="success-message">Mise à jour effectuée avec succès.</p>';
        echo "<script>
                setTimeout(() => {
                    window.location.href = 'liste-product-proprietaire.php';
                }, 3000);
              </script>";
    } else {
