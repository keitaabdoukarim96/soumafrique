...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_stmt_bind_param($stmt_update, 'ssiiisssi', 
    ...
    // Mise à jour des ingrédients (images et noms)
    if (!empty($_FILES['ingredient_images']['name'][0])) {
        foreach ($_FILES['ingredient_images']['name'] as $index => $name) {
            ...
            $ingredient_id = $_POST['ingredient_ids'][$index];
    ...
    if ($update_success) {
        ...
        echo "<script>
        setTimeout(() => {
             window.location.href = 'vew-liste-recette.php';
        }, 3000);
      </script>";
    } else {
    ...
    <div class="flex-1 p-6 flex flex-col items-center justify-center">
        <div class="bg-white shadow-lg rounded-lg p-6 w-full max-w-lg">
            ...
            <form id="edit-recipe-form" method="POST" enctype="multipart/form-data" class="space-y-4">
                <div>
                    <label for="recipe-name" class="block text-gray-700 font-medium mb-2">Nom :</label>
                    <input type="text" id="recipe-name" name="recipe_name" value="<?= htmlspecialchars($recette['recipe_name']) ?>" class="w-full border rounded-lg p-2">
                </div>

                <div>
                    <label for="main-image" class="block text-gray-700 font-medium mb-2">Image principale :</label>
                    <img src="uploads/<?= htmlspecialchars($recette['main_image']) ?>" class="w-full max-h-48 object-cover rounded-lg mb-2">
                    <input type="file" id="main-image" name="main_image" class="w-full border rounded-lg p-2">
                </div>

                <div>
                    <h3 class="text-lg font-bold mb-2">Ingrédients :</h3>
                    <?php foreach ($ingredients as $ingredient) : ?>
                        <div class="ingredient-item flex items-center space-x-2 mb-2">
                            ...
                            <input type="hidden" name="ingredient_old_images[]" value="<?= $ingredient['ingredient_image'] ?>">
                            <input type="text" name="ingredient_titles[]" value="<?= htmlspecialchars($ingredient['ingredient_name']) ?>" class="border rounded-lg p-2">
                            <img src="../admin/<?= htmlspecialchars($ingredient['ingredient_image']) ?>" class="w-16 h-16 object-cover rounded-lg">
                            <input type="file" name="ingredient_images[]" class="border rounded-lg p-2">
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div>
                    <label for="cooking-time" class="block text-gray-700 font-medium mb-2">Durée de cuisson :</label>
                    <input type="number" id="cooking-time" name="cooking_time" value="<?= $recette['cooking_time'] ?>" class="w-full border rounded-lg p-2">
                </div>

                <div>
                    <label for="servings" class="block text-gray-700 font-medium mb-2">Portions :</label>
                    <input type="number" id="servings" name="servings" value="<?= $recette['servings'] ?>" class="w-full border rounded-lg p-2">
                </div>
                <div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    hiddenField.value = <?= json_encode($recette['description']) ?>; // Charger la description existante
